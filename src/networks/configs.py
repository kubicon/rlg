...
@register_torso('mlp')
def _(feature_dim: int = 256, hidden=(256, 256), activation: str = 'relu'):
  from .modules import Activation
  from .torsos import MLPTorso
  return MLPTorso(feature_dim=feature_dim, hidden=tuple(hidden),
                  activation=Activation(kind=activation))


@register_torso('conv')
def _(feature_dim: int = 256, channels=(32, 64, 64),
      kernel_sizes=(8, 4, 3), strides=(4, 2, 1), activation: str = 'relu'):
  from .modules import Activation
  from .torsos import ConvTorso
  return ConvTorso(feature_dim=feature_dim, channels=tuple(channels),
                   kernel_sizes=tuple(kernel_sizes), strides=tuple(strides),
                   activation=Activation(kind=activation))


@register_torso('residual')
def _(feature_dim: int = 256, hidden_dim: int = 256, n_blocks: int = 4,
      activation: str = 'relu'):
  from .modules import Activation
  from .torsos import ResidualTorso
  return ResidualTorso(feature_dim=feature_dim, hidden_dim=hidden_dim,
                       n_blocks=n_blocks, activation=Activation(kind=activation))


@register_torso('resnet')
def _(feature_dim: int = 256, channels=(16, 32, 32),
      blocks_per_stage=(2, 2, 2), activation: str = 'relu'):
  from .modules import Activation
  from .torsos import ResNetTorso
  return ResNetTorso(feature_dim=feature_dim, channels=tuple(channels),
                     blocks_per_stage=tuple(blocks_per_stage),
                     activation=Activation(kind=activation))


@register_torso('transformer')
def _(feature_dim: int = 256, n_heads: int = 4, n_layers: int = 3,
      mlp_dim: int = 512, activation: str = 'gelu',
      norm: str = 'layer', norm_num_groups: int = 32):
  from .modules import Activation, Normalization
  from .torsos import TransformerTorso
  return TransformerTorso(feature_dim=feature_dim, n_heads=n_heads, n_layers=n_layers,
                          mlp_dim=mlp_dim, activation=Activation(kind=activation),
...
def _(action_dim: int, log_std_min: float = -20.0, log_std_max: float = 2.0):
  ...
  return GaussianHead(action_dim=action_dim, log_std_min=log_std_min,
...
def _(torso: dict, head1: dict, head2: dict):
  ...
  return TwinHead(torso=build_torso(torso),
                  head1=build_head(head1), head2=build_head(head2))
...
def _(torso1: dict, head1: dict, torso2: dict, head2: dict):
  ...
  return SeparateTwinHead(torso1=build_torso(torso1), head1=build_head(head1),
                          torso2=build_torso(torso2), head2=build_head(head2))
...
def build_network(data: dict[str, Any]) -> nn.Module:
  ...
  if name not in _network_registry:
    raise ValueError(f"Unknown network '{name}'. Registered: {sorted(_network_registry)}")
