...
class LeducState(NamedTuple):
    ...
    pending_public: jax.Array  # ()         int32  — rank of community card (revealed at flop)
    ...
    call_amount: jax.Array     # ()         float32 — chips owed by cur_player to call (0=no bet)
...
class LeducHoldem(Env):
    ...
    def apply_action(
        ...
        is_second_check = (action == CALL) & (state.call_amount == 0.0) & (state.round_actions >= 1)
