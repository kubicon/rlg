...
class BattleshipState(NamedTuple):
    ...
    shot_grids: jax.Array    # (2, N, N) bool   — shot_grids[p] = cells player p has fired at
...
class Battleship(Env):
    ...
    def _placement_mask(
        self, ship_grid: jax.Array, ship_len: jax.Array
    ) -> jax.Array:
    ...
    def _shooting_step(
        ...
        new_shots = (
            state.shot_grids.at[0, r0, c0].set(True).at[1, r1, c1].set(True)
        )
    ...
    def apply_action(
        ...
        next_state = jax.tree.map(
            lambda a, b: jnp.where(is_place, a, b), s_place, s_shoot
        )
    ...
    def player_observation(
        ...
        opp_hits = (state.shot_grids[p] & (state.ship_grids[q] > 0)).astype(jnp.float32).flatten()
        opp_misses = (state.shot_grids[p] & ~(state.ship_grids[q] > 0)).astype(jnp.float32).flatten()
        ...
        return jnp.concatenate([own_ships, opp_hits, opp_misses, shots_on_me, phase, placed_frac, alive])

    def public_observation(self, state: BattleshipState) -> jax.Array:
        """4*N²+1 float32: both players' hit/miss grids and current phase."""
        p0_hits  = (state.shot_grids[0] & (state.ship_grids[1] > 0)).astype(jnp.float32).flatten()
        p0_miss  = (state.shot_grids[0] & ~(state.ship_grids[1] > 0)).astype(jnp.float32).flatten()
        p1_hits  = (state.shot_grids[1] & (state.ship_grids[0] > 0)).astype(jnp.float32).flatten()
        p1_miss  = (state.shot_grids[1] & ~(state.ship_grids[0] > 0)).astype(jnp.float32).flatten()
        phase = jnp.array([state.phase], dtype=jnp.float32)
    ...
    def state_observation(self, state: BattleshipState) -> jax.Array:
        ...
        p1_miss  = (state.shot_grids[1] & ~(state.ship_grids[0] > 0)).astype(jnp.float32).flatten()
        phase = jnp.array([state.phase], dtype=jnp.float32)
        return jnp.concatenate([p0_ships, p1_ships, p0_hits, p0_miss, p1_hits, p1_miss, phase])
