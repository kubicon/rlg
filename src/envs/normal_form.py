...
class NormalFormGame(Env):
    ...
    def player_observation(self, state: NormalFormState, player_id: jax.Array) -> jax.Array:
    ...
    def legal_actions(self, state: NormalFormState, player_id: jax.Array | int) -> jax.Array:
        return jnp.ones(self._n, dtype=jnp.bool_)

    # ── Perfect-recall representations ──────────────────────────────────────

    def information_set(self, state: NormalFormState, player_id: jax.Array | int) -> jax.Array:
