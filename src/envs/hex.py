...
class Hex(Env):
    ...
    def apply_action(
        ...
        if self.dark:
            ...
            # Non-abrupt: keep the turn when blocked so the player acts again.
            switch = cell_empty if not self.abrupt else True
        else:
        ...
        new_state = HexState(
            ...
            cur_player=jnp.where(won | ~switch, state.cur_player, jnp.int32(1 - cp)),
            step=state.step + 1,
            done=won,
        )
    ...
    def information_set(
        ...
        # Dark Hex: own_placed + blocked encodes complete private history.
