...
def main(checkpoint_dir: str) -> None:
    ...
    for path in paths:
        ...
        p0_str = " ".join(f"{p:.3f}" for p in p0_init) if p0_init is not None else "not found"
        p1_str = " ".join(f"{p:.3f}" for p in p1_init) if p1_init is not None else "not found"
        ...
        print(f"{ckpt_id:>6}  {step:>8}  {bv.v0_p0_br:>9.4f}  {bv.v1_p1_br:>9.4f}  {nashconv:>10.4f}")
        results.append({
            "ckpt_id": ckpt_id,
            "step": step,
